CREATE TABLE `assignment` (
	...
	FOREIGN KEY (`employment_id`) REFERENCES `employment`(`id`) ON UPDATE no action ON DELETE no action,
...
CREATE TABLE `lifecycle_event` (
	...
	`occurred_at` text NOT NULL,
	FOREIGN KEY (`person_id`) REFERENCES `person`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`transaction_request_id`) REFERENCES `transaction_request`(`id`) ON UPDATE no action ON DELETE no action,
