...
CREATE TABLE `__new_p2list_audit_event` (
	...
	CONSTRAINT "p2list_audit_event_duration_ms_bounded" CHECK("__new_p2list_audit_event"."duration_ms" between 0 and 600000),
...
CREATE UNIQUE INDEX `p2list_audit_event_correlation_type_unique` ON `p2list_audit_event` (`correlation_id`,`event_type`);--> statement-breakpoint
CREATE INDEX `p2list_audit_event_correlation_occurred_idx` ON `p2list_audit_event` (`correlation_id`,`occurred_at`);
