-- Extend the bounded transaction_request payload contract for MVP-C
-- termination while preserving request-dependent foreign-key rows.
CREATE TEMP TABLE `__termination_migration_lifecycle_event_rows` AS SELECT "id", "person_id", "transaction_request_id", "contact_point_id", "event_type", "effective_date", "occurred_at" FROM `lifecycle_event`;
--> statement-breakpoint
CREATE TEMP TABLE `__termination_migration_onboarding_apply_job_attempt_rows` AS SELECT "id", "transaction_request_id", "person_id", "status_code", "attempted_at", "worker_id", "correlation_id", "retryable", "error_message" FROM `onboarding_apply_job_attempt`;
--> statement-breakpoint
...
CREATE TABLE `__new_transaction_request` (
	...
	CONSTRAINT "transaction_request_payload_version_allowed" CHECK("__new_transaction_request"."payload_version" is null or "__new_transaction_request"."payload_version" in ('mvp_a_onboarding_v1', 'mvp_b_transfer_v1', 'mvp_c_termination_v1')),
...
--> statement-breakpoint
INSERT INTO `lifecycle_event`("id", "person_id", "transaction_request_id", "contact_point_id", "event_type", "effective_date", "occurred_at") SELECT "id", "person_id", "transaction_request_id", "contact_point_id", "event_type", "effective_date", "occurred_at" FROM `__termination_migration_lifecycle_event_rows`;
--> statement-breakpoint
...
--> statement-breakpoint
INSERT INTO `onboarding_apply_job_attempt`("id", "transaction_request_id", "person_id", "status_code", "attempted_at", "worker_id", "correlation_id", "retryable", "error_message") SELECT "id", "transaction_request_id", "person_id", "status_code", "attempted_at", "worker_id", "correlation_id", "retryable", "error_message" FROM `__termination_migration_onboarding_apply_job_attempt_rows`;
--> statement-breakpoint
...
--> statement-breakpoint
DROP TABLE `__termination_migration_lifecycle_event_rows`;
--> statement-breakpoint
DROP TABLE `__termination_migration_onboarding_apply_job_attempt_rows`;
