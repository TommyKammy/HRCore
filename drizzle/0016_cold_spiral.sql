...
CREATE TABLE `csv_import_row_outcome` (
	...
	CONSTRAINT "csv_import_row_outcome_status_allowed" CHECK("csv_import_row_outcome"."status_code" in ('applied', 'failed')),
	...
	CONSTRAINT "csv_import_row_outcome_status_payload_pair" CHECK(("csv_import_row_outcome"."status_code" = 'applied' and "csv_import_row_outcome"."transaction_request_id" is not null and "csv_import_row_outcome"."lifecycle_event_id" is not null and "csv_import_row_outcome"."error_message" is null) or ("csv_import_row_outcome"."status_code" = 'failed' and "csv_import_row_outcome"."transaction_request_id" is null and "csv_import_row_outcome"."lifecycle_event_id" is null and "csv_import_row_outcome"."error_message" is not null and length("csv_import_row_outcome"."error_message") > 0))
);
--> statement-breakpoint
CREATE UNIQUE INDEX `csv_import_row_outcome_row_unique` ON `csv_import_row_outcome` (`row_id`);--> statement-breakpoint
