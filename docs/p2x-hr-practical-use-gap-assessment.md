...
| production authorization/RLS | Bounded actor, subject, tenant, and environment checks are not production RBAC/RLS authority.                                                        | Required before any stronger claim: accepted authorization/data-scope design, role binding, trusted proxy identity boundary, PostgreSQL RLS source of truth, and negative enforcement tests. |
| raw payload and broad export | Existing gates intentionally block unrestricted raw payload and broad CSV/export.                                                                    | Separate raw-view/export permission evidence, redaction and masking profile, template allowlist, watermark or manifest, download-log evidence, and legal approval.                           |
