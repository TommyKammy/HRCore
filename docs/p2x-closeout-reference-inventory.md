...
| -------------------------------------------------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `README.md` P2X link cluster                                   | needs narrow cleanup | Current P2X-01/P2X-02 links are present. Add this inventory link so future readers find the post-P2X-02 stale wording scan without relying on issue text.                                                                                                                           |
| `README.md` baseline structure P2X cluster                     | needs narrow cleanup | P2X-01 and P2X-02 summaries are current, but the baseline list should also mention this inventory as guard-oriented closeout synchronization evidence.                                                                                                                              |
...
| `src/repository-guards.test.ts` P2X guard cluster              | needs narrow cleanup | Existing P2X guards protect the P2X-01/P2X-02 documents and child artifacts. Add this inventory guard so the accepted P2X-02 boundary cannot disappear from closeout synchronization work.                                                                                          |
| `src/mvp-a-policy-as-code-ci.ts` monitored documentation paths | needs narrow cleanup | Policy-as-code already monitors the P2X closeout and bounded follow-up artifacts. Add this inventory path so stronger-readiness overclaims in the inventory fail closed.                                                                                                            |
...
- Add the README discovery link for this inventory.
- Add this inventory to policy-as-code monitored documentation paths.
- Add the focused repository guard for this inventory.
