...
const p2xBlockedSubjectPatterns: Array<[string, RegExp]> = [
  ...
  [
    "production audit/archive readiness",
    /production\s+audit\s+(?:readiness|archive)|production\s+support\s+audit\s+search|hash-chain\/archive\s+design(?:\s+acceptance)?|audit\s+retention\s+posture|restore\s+evidence|tamper-evidence\s+verification|broad\s+audit\s+search|compliance\s+archive|WORM(?:\/Object\s+Lock)?|Object\s+Lock/iu,
  ],
  [
    "production backup/restore readiness",
