...
function p2xBoundedPracticalUseArtifactOverclaimClaims(
  ...
  const prohibitedClaims: Array<[string, RegExp]> = [
    ...
    [
      ...
      /\b(?:retention\/deletion(?:\s+(?:runtime|jobs?|requests?|ADR\s+evidence))?|anonymization(?:\s+jobs?)?|hard[-\s]+delete(?:\s+jobs?)?|deletion[-\s]+jobs?|physical\s+deletion(?:\s+(?:exception|path|handling))?|legal[-\s]+hold(?:\s+workflow)?|deletion[-\s]+job\s+custody|retention[-\s]+log(?:\s+runtime)?|restore\s+cleanup|no-orphan\s+tests?|jurisdiction\/legal-entity\s+applicability)\b[^.;]{0,60}\b(?:ready|allowed|approved|accepted|go|enabled|available|complete)\b|\b(?:ready|approved|accepted|go|enabled|available|complete)\b[^.;]{0,60}\b(?:retention\/deletion(?:\s+(?:runtime|jobs?|requests?|ADR\s+evidence))?|anonymization(?:\s+jobs?)?|hard[-\s]+delete(?:\s+jobs?)?|deletion[-\s]+jobs?|physical\s+deletion(?:\s+(?:exception|path|handling))?|legal[-\s]+hold(?:\s+workflow)?|deletion[-\s]+job\s+custody|retention[-\s]+log(?:\s+runtime)?|restore\s+cleanup|no-orphan\s+tests?|jurisdiction\/legal-entity\s+applicability)\b/iu,
    ],
    [
      "future-extension readiness",
      /\b(?:future[-\s]+extension(?:\s+(?:runtime|readiness|schema|API|surface|surfaces?))?|extension\s+scope\s+records?|migration\/runtime\s+authorization|schema\/API\/runtime\s+authorization|negative\s+no-escape-hatch\s+tests?|prohibited[-\s]+payload\s+runtime|privacy[-\s]+classification(?:\s+runtime)?|consent\s+runtime|employment[-\s]+status\s+runtime|work[-\s]+arrangement\s+runtime|future[-\s]+date\s+worker\s+authority|extension\s+schema|extension\s+API)\b[^.;]{0,60}\b(?:ready|allowed|approved|accepted|go|enabled|available|complete)\b|\b(?:ready|approved|accepted|go|enabled|available|complete)\b[^.;]{0,60}\b(?:future[-\s]+extension(?:\s+(?:runtime|readiness|schema|API|surface|surfaces?))?|extension\s+scope\s+records?|migration\/runtime\s+authorization|schema\/API\/runtime\s+authorization|negative\s+no-escape-hatch\s+tests?|prohibited[-\s]+payload\s+runtime|privacy[-\s]+classification(?:\s+runtime)?|consent\s+runtime|employment[-\s]+status\s+runtime|work[-\s]+arrangement\s+runtime|future[-\s]+date\s+worker\s+authority|extension\s+schema|extension\s+API)\b/iu,
    ],
    [
...
const p2xBlockedSubjectPatterns: Array<[string, RegExp]> = [
  ...
  [
    "retention/deletion runtime readiness",
    /retention\/deletion(?:\s+(?:runtime|jobs?|requests?|ADR\s+evidence))?|anonymization(?:\s+jobs?)?|hard[-\s]+delete(?:\s+jobs?)?|deletion[-\s]+jobs?|physical\s+deletion(?:\s+(?:exception|path|handling))?|legal[-\s]+hold(?:\s+workflow)?|deletion[-\s]+job\s+custody|retention[-\s]+log(?:\s+runtime)?|restore\s+cleanup|no-orphan\s+tests?|jurisdiction\/legal-entity\s+applicability/iu,
  ],
  [
    "future-extension readiness",
    /future[-\s]+extension(?:\s+(?:runtime|readiness|schema|API|surface|surfaces?))?|extension\s+scope\s+records?|migration\/runtime\s+authorization|schema\/API\/runtime\s+authorization|negative\s+no-escape-hatch\s+tests?|prohibited[-\s]+payload\s+runtime|privacy[-\s]+classification(?:\s+runtime)?|consent\s+runtime|employment[-\s]+status\s+runtime|work[-\s]+arrangement\s+runtime|future[-\s]+date\s+worker\s+authority|extension\s+schema|extension\s+API/iu,
  ],
  [
    "broad export readiness",
