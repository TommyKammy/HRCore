...
export function applyDueTerminationTransactionRequests(
  ...
  const candidates = readDueTerminationApplyCandidates(
    db,
    batchLimit,
    effectiveDate,
  );
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error);
      const retryable = isRetryableTerminationApplyWorkerFailure(error);
...
function parseApplyDueTerminationTransactionRequestsInput(
  input: unknown,
...
function readDueTerminationApplyCandidates(
  db: OnboardingTransactionRequestDatabase,
  batchLimit: number,
